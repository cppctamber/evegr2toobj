...
void __cdecl log(int a, int b, char* msg) { fprintf(stderr, "Log (%d %d) %s\n", a, b, msg); }
...
int getTypeSize(int type) {
    switch (type) {
    case 10:
        return 4; // 32bit float
    case 11: // granny_int8
    case 12: // granny_uint8
    case 14:
        return 1; // granny_uint8
    case 21:
        return 2; // 16bit float
    default:
        return 0;
    }
}
...
float convertDummy(uint8_t* value) { return 0.0f; }

float convert10(uint8_t* value) { return *(float*)value; }

float convert11(uint8_t* value) { return *(granny_int8*)value; }

float convert12or14(uint8_t* value) { return *(granny_uint8*)value; }
...
void getMeshInfo(const Mesh_t* mesh) {
    ...
    for (uint32_t i = 0; i < nVertexComponents_; ++i) {
        ...
        vertexComponents_[i].convert = (vertexComponents_[i].exists)
            ? getConvertFunc(sm[vertexComponents_[i].index].Type)
            : &convertDummy;
        vertexComponents_[i].offset
            = (vertexComponents_[i].exists) ? offsets[vertexComponents_[i].index] : 0;
        vertexComponents_[i].typeSize
            = (vertexComponents_[i].exists) ? getTypeSize(sm[vertexComponents_[i].index].Type) : 0;
    }
...
void convertMeshToObj(const Mesh_t* mesh, FILE* file, int* nMeshOffset) {
    ...
    if (vertexComponents_[5].exists) {
        printf("-- TexCoord1 (unsupported)\n");
    }
    if (vertexComponents_[2].exists) {
        printf("-- Tangent   (unsupported)\n");
    }
    if (vertexComponents_[3].exists) {
        printf("-- Binormal  (unsupported)\n");
    }
    ...
    if (bytesPerIndex_ == 2) {
        ...
        for (int iGroup = 0; iGroup < nTriangleGroups_; ++iGroup) { // for each surface
            ...
            for (int iTriangle = 0; iTriangle < triangleGroups_[iGroup].TriCount; ++iTriangle) {
                ...
                index1 = indices[index0] + *nMeshOffset;
                index2 = indices[index0 + 1] + *nMeshOffset;
                index3 = indices[index0 + 2] + *nMeshOffset;
                fprintf(file, "f %d/%d/%d %d/%d/%d %d/%d/%d\n", index1, index1, index1, index2,
                    index2, index2, index3, index3, index3);
            }
        }
...
int main(int argc, char** argv) {
    ...
    for (int iMesh = 0; iMesh < nMeshes; ++iMesh) {
        ...
        printf("Mesh: %s (%d)\n", name, iMesh);
        getMeshInfo(mesh);
        convertMeshToObj(mesh, file, &nMeshOffset);
        printf("\n");
    }
    fclose(file);
